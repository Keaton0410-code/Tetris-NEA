...
# Cached fonts avoids reloading the same size repeatedly
_font_cache = {}
_font_warning_shown = False

class App:
    """Solo Tetris app for human practice (arrow keys). """
    def __init__(self):
    ...
    def load_sprites(self):
        #Grab every .png in the sprite folder
    ...
    def check_events(self):
        #Reset each frame timers below will set them to True when they fire
        self.animation_trigger = False
        self.fast_animation_trigger = False
...
def get_font(font_size):
    """Returns a font size.  Tries the custom .ttf first,
    falls back to pygame's built in. font is cached.
    """
    global _font_cache, _font_warning_shown
    #Already loaded this size hand it back 
    ...
    try:
        ...
        #Only warn once so the console doesn't flood
...
def make_background(width, height, colour1, colour2):
    """Builds a vertical gradient surface from color1 (top) to color2 (bottom)."""
...
def draw_button(surface, rect, text, font, is_hovering, base_color=(70, 130, 180), hover_color=(100, 160, 210)):
    ...
    # Swap colour when the mouse is over the button
...
class Button:
    """Clickable button that changes colour on hover."""
...
def main_menu():
    ...
    #Shared layout values for all buttons
    button_width = 400
    button_height = 60
    start_y = 200
    spacing = 80
    ...
    # Each menu option gets its own colour theme
    buttons = {
    ...
    while True:
        ...
        #Title rendered with a drop shadow
        ...
        #refresh hover state then paint every button
