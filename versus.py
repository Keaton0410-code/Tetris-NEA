# versus.py - SIMPLIFIED SCALABLE VERSION
# Uses standard TILE_SIZE but calculates proper positions
...
class VersusApp:
    def __init__(self, total_players=2, cpu_opponents=1, cpu_difficulty="medium", player_names=None):
        ...
        board_width = FIELD_W * TILE_SIZE  # 500px
        board_height = FIELD_H * TILE_SIZE  # 1000px
        
        # Calculate layout
        if self.total_players == 2:
            # 2 boards: generous spacing
            margin_left = 100
            margin_right = 100
            gap = 100
            window_width = margin_left + board_width + gap + board_width + margin_right
            # = 100 + 500 + 100 + 500 + 100 = 1300px
        else:  # 3 boards
            # 3 boards: fit in reasonable width
            margin_left = 80
            margin_right = 80
            gap = 70
            window_width = margin_left + (board_width * 3) + (gap * 2) + margin_right
            # = 80 + 1500 + 140 + 80 = 1800px (fits in 1920px screen)
      ...
      #  print(f"Window: {window_width}×{window_height}")
       ...
       # print(f"Board size: {board_width}×{board_height}")
      #  print(f"Margin left: {margin_left}, Gap: {gap}")
        ...
        for board_index in range(self.total_players):
            # Calculate board position in pixels
            board_x = margin_left + board_index * (board_width + gap)
            board_y = margin_top
            
            # Convert to tiles for Tetris class (it expects tile offsets)
            ...
            if right_edge > window_width:
                print(f" Board exceeds window width!")
            
            # Store pixel position for UI
